...
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('username','password') displayInfo=realm.password && realm.registrationAllowed && !registrationDisabled??; section>
    <#if section = "header">
        <span class="aip-zh">${msg("loginAccountTitle")}</span>
        <span class="aip-en">${msg("loginAccountTitleEn")}</span>
    <#elseif section = "form">
        <div id="kc-form">
                ...
                <form id="kc-form-login" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
                    <#if !usernameHidden??>
                        <div class="${properties.kcFormGroupClass!}">
                            <label for="username" class="${properties.kcLabelClass!}">
                                <span class="aip-zh"><#if !realm.loginWithEmailAllowed>${msg("username")}<#elseif !realm.registrationEmailAsUsername>${msg("usernameOrEmail")}<#else>${msg("email")}</#if></span>
                                <span class="aip-en"><#if !realm.loginWithEmailAllowed>${msg("usernameEn")}<#elseif !realm.registrationEmailAsUsername>${msg("usernameOrEmailEn")}<#else>${msg("emailEn")}</#if></span>
                            </label>
                    ...
                    <div class="${properties.kcFormGroupClass!}">
                        <label for="password" class="${properties.kcLabelClass!}">
                            <span class="aip-zh">${msg("password")}</span>
                            <span class="aip-en">${msg("passwordEn")}</span>
                        </label>
